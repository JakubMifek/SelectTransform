...
export class Helper {
     ...
     * @param {any} new_val
     */
    static resolve(o, path, new_val) {
        if (path && path.length > 0) {
            const func = Function("new_val", `with(this) {this${path}=new_val; return this;}`).bind(o);
            return func(new_val);
        }
        else {
            o = new_val;
     ...
     */
    static fillout(template, data, raw) {
        ...
        if (!variables)
            return template;
        if (raw)
            return Helper._fillout(null, variables[0], data);
    ...
    static _fillout(template, variable, data) {
